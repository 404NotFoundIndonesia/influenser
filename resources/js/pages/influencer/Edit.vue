...
import { Influencer, Platform } from '@/types/model';
import { ref } from 'vue';
...
const form = useForm<InfluencerForm>({
    ...
    photo: null,
    keyOpinionLeaders: (props.item?.key_opinion_leaders ?? []) as KeyOpinionLeaderForm[],
    niches: props.item.niches?.map(n => n.id) ?? [],
});
...
const removeKOL = (event: MouseEvent, index: number) => {
    confirm.require({
        ...
        accept: () => {
            const kol = form.keyOpinionLeaders[index];
            if (kol.id === null) {
                setTimeout(() => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <div>
                    <template v-if="item.profile_picture_path && !editPhoto">
                        <div class="rounded rounded-border flex flex-col border border-surface items-center gap-2">
                            <div class="flex justify-end items-center flex-1 p-[1.125rem] w-full">
                                <Button @click="() => editPhoto = true" icon="pi pi-pencil" rounded outlined severity="secondary"></Button>
                            </div>
                            <div class="relative">
                    ...
                    <FileUpload
                        ...
                        <template #header="{ chooseCallback }">
                            <div class="flex flex-wrap justify-between items-center flex-1 gap-4">
                                <Button @click="chooseCallback()" icon="pi pi-images" rounded outlined severity="secondary" :disabled="form.photo !== null"></Button>
                                <Button @click="() => editPhoto = false" icon="pi pi-times" rounded outlined severity="danger" :disabled="form.photo !== null"></Button>
                            </div>
                        </template>
